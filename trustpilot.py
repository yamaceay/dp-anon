...
def build_mechanism(args: argparse.Namespace) -> dpmlm.DPMechanism:
    ...
    if args.type == "dpmlm" and risk_type != "uniform":
        if not args.dpmlm_risk_model:
            logger.error("Non-uniform DPMLM risk scoring requires --dpmlm-risk-model with a text-classification model.")
...
def main():
    ...
    privatizer = build_mechanism(args)

    text = ds['train'][0]['description']
    result = privatizer(text)

    print("\n" + "="*80)
    print("DIFFERENTIAL PRIVACY TEXT PROCESSING RESULTS")
    print("="*80)
    print(f"Original Text (len={len(result.original_text)}):")
    print(result.original_text)
    print("-"*80)
    print(f"Anonymized Text (len={len(result.private_text)}): ")
    print(result.private_text)
    print("-"*80)
    print(f"Token Statistics:")
    print(f"  - Total Tokens: {result.total_tokens}")
    print(f"  - Perturbed Tokens: {result.perturbed_tokens}")
